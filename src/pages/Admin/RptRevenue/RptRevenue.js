...
import styles from './RptRevenue.module.scss';
import { useMemo, useState } from 'react';
import { Navigate } from 'react-router-dom';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faMagnifyingGlass } from '@fortawesome/free-solid-svg-icons';
import Loader from '~/components/Loader';
import { AdminReportRevenueTable } from '~/components/Table';
...
function RptRevenue() {
    ...
    const hadleClickSearch = () => {
        ...
        setLoading(true);
        fetch('/Report/WA_Rpt_TourRevenue', requestOptions)
            .then((response) => response.json())
            .then((result) => {
                setReport(result.Data.Lst_Rpt_TourRevenue);
                setLoading(false);
            })
            ...
            return (
                <div className={cx('wrapper')}>
                    <div className={cx('inner')}>
                        ...
                        <div>
                            <h3>Tổng doanh thu: {report && formatMoney(caculateTotal(currentData), 0)} đ</h3>
                        </div>
                        ...
                        <section className={cx('table-sec')}>
                            <AdminReportRevenueTable data={currentData} rowsPerPage={5} />
                        </section>
                    </div>
                    <ToastContainer />
                </div>
